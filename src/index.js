...
async function generateToken(options) { 
    ...
    if (response.statusCode || !tokenresponse.id || !tokenresponse.secret) { 
 ...
 */
async function makeAuthenticatedFetch(credentials, fetch) { 
    const authFetch = await createAuthenticatedFetchFunction(credentials, fetch);
    return authFetch
}

async function createAuthenticatedFetchFunction(credentials, passedFetch) { 
